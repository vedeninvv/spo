...
int translate_literal(processedLiteral literal) {
...
int translate_expression(processedExpression expression, symbolicTable *table);

int translate_binary(processedBinary binary, symbolicTable *table) {
...
int translate_call(processedCall call, symbolicTable *table) {
...
int translate_return(processedExpression expression, symbolicTable *table) {
...
int translate_unary(processedUnary unary, symbolicTable *table) {
...
int translate_variable(processedVar var, symbolicTable *table) {
    char *label;
    if (var.label != NULL) {
        label = var.label;
    } else {
...
int translate_block(processedBlock block, symbolicTable *table, char *lastGoThroughLabel);

int translate_statement(processedStatement statement, symbolicTable *table, char *lastGoThroughLabel);

int translate_ifs(processedIf ifs, symbolicTable *table, char *lastGoThroughLabel) {
    put_comment("if")
...
int translate_while(processedWhile whilep, symbolicTable *table) {
    put_comment("while")
...
int translate_dowhile(processedDoWhile doWhile, symbolicTable *table) {
    put_comment("doWhile")
    char *loopbackLabel = labelName();
    put_label(loopbackLabel)
...
int translate_assigment(processedAssigment assigment, symbolicTable *table) {
...
int translate_procedure(int num, processedFunc *funcs, int count, symbolicTable *table) {
    table = newSymbolicTable(table);
    symbol *s = symbolicTable_findSymbol(table, funcs[num].identifier);
    processedFunc func = funcs[num];
    put_label(s->label)
