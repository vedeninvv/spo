...
int compareTypes(processedType type1, processedType type2) {
...
int processedStatement_visit(processedStatement statement, symbolicTable *table);

int processedExpression_visit(processedExpression expression, symbolicTable *table, processedType *expectedType,
    processedType *pType);

int processedBlock_visit(processedBlock block, symbolicTable *table) {
    int countVars = 0;
    ...
    for (int i = 0; i < block.statementsCount; ++i) {
        if (processedStatement_visit(block.statements[i], table) != 0) {
...
int processedVars_visit(processedVars vars, symbolicTable *table) {
    for (int i = 0; i < vars.count; ++i) {
        if (vars.vars[i].initValue != NULL) {
            if (processedExpression_visit(*vars.vars[i].initValue, table, &vars.vars[i].type, NULL) != 0) {
                return 2;
            }
        }
        union ctx c = {};
...
int
processedBinary_visit(processedBinary binary, symbolicTable *table, processedType *expectedType, processedType *foundType) {
    processedType type = {};
    symbol *s;
    ...
    processedType foundTypeLeft = {};
    if (processedExpression_visit(*binary.leftOperand, table, expectedType, &foundTypeLeft) != 0) {
        return 1;
    }
    if (foundType != NULL) {
    ...
    return processedExpression_visit(*binary.rightOperand, table, expectedType, NULL);
}

int processedUnary_visit(processedUnary unary, symbolicTable *table, processedType *expectedType, processedType *foundType) {
    return processedExpression_visit(*unary.operand, table, expectedType, foundType);
}

int processSemanticsForFunction(processedFunc *func, symbolicTable *table);

int processedExpression_visit(processedExpression expression, symbolicTable *table, processedType *expectedType,
    processedType *foundType) {

    symbol *s;
    int result = 0;
    switch (expression.type) {
        case UNARY:
            result = processedUnary_visit(expression.unary, table, expectedType, foundType);
            break;
        case BINARY:
            result = processedBinary_visit(expression.binary, table, expectedType, foundType);
            break;
        case BRACES:
            result = processedExpression_visit(*expression.expression, table, expectedType, foundType);
            break;
        case CALL:
            ...
            for (int i = 0; i < expression.call.argumentExpressions.expressionsCount; ++i) {
                processedExpression_visit(expression.call.argumentExpressions.expressions[i], table,
            ...
            break;
        case INDEXER:
            result = processedExpression_visit(*expression.indexer.expression, table, NULL, foundType);
            break;
        case PLACE:
...
processedType bool = {.type =  BOOL};

int processedWhile_visit(processedWhile whilep, symbolicTable *table) {
    if (processedExpression_visit(whilep.condition, table, &bool, NULL) != 0) {
        return 1;
    }
    return processedBlock_visit(whilep.block, table);
}

int processedDoWhile_visit(processedDoWhile dowhile, symbolicTable *table) {
    if (processedExpression_visit(dowhile.condition, table, &bool, NULL) != 0) {
        return 1;
    }
    return processedBlock_visit(dowhile.block, table);
}

int processedIf_visit(processedIf *ifp, symbolicTable *table) {
    if (processedExpression_visit(ifp->statement.condition, table, &bool, NULL) != 0) {
        return 1;
    }
    if (processedStatement_visit(ifp->statement.statement, table) != 0) {
        return 1;
    }
    if (ifp->elseStatementExists == 1) {
        if (processedStatement_visit(ifp->elseStatement, table) != 0) {
            return 1;
        }
    }
    return 0;
...
int processedAssigment_visit(processedAssigment assigment, symbolicTable *table) {
    symbol *s;
    switch (assigment.to.type) {
        case PLACE:
            ...
            return processedExpression_visit(*assigment.rightPart, table, &s->type, NULL);
        case INDEXER:
...
int processedStatement_visit(processedStatement statement, symbolicTable *table) {
    ...
    switch (statement.type) {
        case STATEMENT_TYPE_ASSIGMENT:
            return processedAssigment_visit(statement.assigment, table);
        case STATEMENT_TYPE_BLOCK:
            return processedBlock_visit(statement.block, table);
        case STATEMENT_TYPE_VAR:
            return processedVars_visit(statement.vars, table);
        case STATEMENT_TYPE_IF:
            return processedIf_visit(statement.ifp, table);
        case STATEMENT_TYPE_WHILE:
            return processedWhile_visit(statement.whilep, table);
        case STATEMENT_TYPE_DO_WHILE:
            return processedDoWhile_visit(statement.dowhile, table);
        case STATEMENT_TYPE_BREAK:
            break;
        case STATEMENT_TYPE_EXPRESSION:
            return processedExpression_visit(statement.expression, table, NULL, NULL);
        case STATEMENT_TYPE_RETURN:
            ...
            return processedExpression_visit(statement.expression, table, &s->ctx.func->returnType, NULL);
    }
    return 0;
}
...
int processSemanticsForFunction(processedFunc *func, symbolicTable *table) {
    ...
    if (processedBlock_visit(func->body, table) != 0) {
        symbolicTable_free(table);
        return 1;
    }
    symbolicTable_free(table);
